...
int main(int argc, char **argv) {
	...
	if(coords[1] > 0) { // if we have a neighbor to the left
		...
		MPI_Irecv(R_Col_recv, N, MPI_FLOAT, dest_rank, 1, comm, &recv_req[0]);
	} // if(coords[1] != 0) {
	...
	if(coords[1] < (dims[1]-1)) {	// if we have a neighbor to the right 
		...
		MPI_Irecv(L_Col_recv, N, MPI_FLOAT, dest_rank, 1, comm, &recv_req[1]);
	} // if(coords[1] != dims[1]) {
	...
	if(coords[0] > 0) {	// If we have a neighbor above
		...
		MPI_Irecv(B_Row_recv, N, MPI_FLOAT, dest_rank, 1, comm, &recv_req[2]);
	} // if(coords[0] != 0) {
	...
	if(coords[0] < (dims[0]-1)) { // if we have a neighbor below
		...
		MPI_Irecv(T_Row_recv, N, MPI_FLOAT, dest_rank, 1, comm, &recv_req[3]);
	} // if(coords[0] != 0) {
